...
function replaceText(word, replacement, text) {
  ...
  const regaxp = new RegExp(word, "g");
  return text.replace(regaxp, replacement);
}
...
function checkWord(word, text) {
  ...
  const regaxp = new RegExp(word, "i");
  return regaxp.test(text);
}
...
function extractTextInParentheses(str) {
  ...
  const matches = Array.from(str.matchAll(regaxp));
...
function countEmails(str) {
  ...
  const matches = Array.from(str.match(email));
...
function checkFlags(regex) {
  ...
  const arr = flags.includes("i") ? ["ignoreCase"] : [];
